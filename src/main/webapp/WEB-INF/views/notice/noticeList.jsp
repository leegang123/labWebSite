...
<div class="container">
    ...
    <table class="table">
        ...
        <c:forEach varStatus="st" var="notice" items="${noticeList}">
            ...
            <c:if test="${notice.noticeTop == null}">
        ...
        <tr style="height: 30px; ">
            ...
            <td colspan="4" style="text-align: center">
                <c:if test="${startPage <= 1}">
                    <button onclick="location.href='${contextPath}/noticeList.do?page=${startPage - 1}'" class="btn btn-danger" disabled>이전</button>
                </c:if>
                <c:if test="${startPage > 1}">
                    <button onclick="location.href='${contextPath}/noticeList.do?page=${startPage - 1}'" class="btn btn-danger">이전</button>
                </c:if>
                <c:forEach var="page" begin="${startPage}" end="${endPage}">
                    <a href="${contextPath}/noticeList.do?page=${page}">${page}</a>
                </c:forEach>
                <c:if test="${startPage + 10 >= totalPage}">
                    <button onclick="location.href='${contextPath}/noticeList.do?page=${endPage + 1}'" class="btn btn-danger" disabled>다음</button>
                </c:if>
                <c:if test="${startPage + 10 < totalPage}">
                    <button onclick="location.href='${contextPath}/noticeList.do?page=${endPage + 1}'" class="btn btn-danger">다음</button>
                </c:if>
            </td>
            <td>
                <input type="button" value="글쓰기" class="btn btn-info btn-sm pull-right" onclick="location.href='${contextPath}/noticeWriteForm.do'">
            </td>
        </tr>
    </table>
        <!-- Modal -->
